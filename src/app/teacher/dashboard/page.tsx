...
import { useGetTeacherDashboard } from "@/queries/useDashboardQuery";
...
export default function TeacherDashboardPage() {
    ...
    const { data: dashData, isLoading: dashLoading } = useGetTeacherDashboard();
    const [dialogOpen, setDialogOpen] = useState(false);

    const teacher = profileData?.data;
    ...
    const scoreChartData = dashboard ? [
        { name: "Xuất sắc", total: dashboard.scoreDistribution.excellent, color: "#f43f5e" },
        { name: "Giỏi", total: dashboard.scoreDistribution.good, color: "#ef4444" },
        { name: "Khá", total: dashboard.scoreDistribution.average, color: "#dc2626" },
        { name: "TB", total: dashboard.scoreDistribution.weak, color: "#991b1b" },
        { name: "Yếu", total: dashboard.scoreDistribution.poor, color: "#7f1d1d" },
    ] : [];
    ...
    return (
        <div className="space-y-8">
                    ...
                    <div className="flex flex-col xl:flex-row items-center xl:items-start gap-10 relative z-10">
                        ...
                        <div className="flex-1 space-y-8 w-full">
                            <div className="flex flex-col md:flex-row justify-between items-center md:items-start gap-6 text-center md:text-left">
                                <div className="space-y-2">
                                    ...
                                    <div className="flex flex-wrap justify-center md:justify-start gap-3">
                                        ...
                                        {(teacher?.warningCount ?? 0) > 0 && (
                                            <Badge className="bg-red-100 text-red-600 border-none font-black rounded-full px-4 py-1">
                                                <AlertTriangle className="size-3 mr-1" />
                                                Cảnh cáo: {teacher?.warningCount}
                                            </Badge>
                                        )}
                                        {dashboard?.pendingScheduleChanges != null && dashboard.pendingScheduleChanges > 0 && (
                                            <Badge className="bg-amber-100 text-amber-700 border-none font-black rounded-full px-4 py-1">
                                                <Clock className="size-3 mr-1" />
                                                {dashboard.pendingScheduleChanges} đơn đổi lịch
                                            </Badge>
                                        )}
                                    </div>
                                </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                ...
                                <InfoItem icon={Briefcase} label="Lương cơ bản" value={teacher?.salaryRate ? formatCurrency(teacher.salaryRate) : "Chưa cập nhật"} />
                                ...
                                <InfoItem icon={BookOpen} label="Tổng bộ môn" value={`${teacher?.teacherSpecialties?.length || 0} môn`} />
                            </div>
                        </div>
                    </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                {[
                    { label: "Tổng buổi đã dạy", value: dashboard?.totalSessions ?? 0, color: "text-red-600", bg: "bg-red-50", icon: Clock, desc: "tất cả các lớp" },
                    { label: "Thu nhập", value: dashboard?.totalIncome ? formatCurrency(dashboard.totalIncome) : "0", color: "text-red-600", bg: "bg-red-50", icon: DollarSign, desc: "tổng thu nhập" },
                    { label: "Lớp phụ trách", value: dashboard?.totalClasses ?? 0, color: "text-red-600", bg: "bg-red-50", icon: BookOpen, desc: "đang hoạt động" },
                    { label: "Học viên", value: dashboard?.totalStudents ?? 0, color: "text-red-600", bg: "bg-red-50", icon: Users, desc: "đang quản lý" },
                ].map((stat, i) => (
                ...
                <Card className="rounded-[2.5rem] border-[var(--sidebar-border)] shadow-sm overflow-hidden bg-white">
                    ...
                    <CardContent className="p-8 h-[300px] flex flex-col items-center justify-center">
                        ...
                        <p className="text-xs font-bold text-slate-400 mt-6">
                            Dựa trên điểm danh các buổi học trong tháng
                        </p>
                    </CardContent>
                </Card>
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                    ...
                    <div className="space-y-4">
                        {dashboard?.todaySessions && dashboard.todaySessions.length > 0 ? (
                                        ...
                                        <div className="flex items-center gap-6">
                                            ...
                                            <div>
                                                ...
                                                <div className="flex flex-wrap items-center gap-4 text-xs font-bold text-muted-foreground mt-1">
                                                    <span className="flex items-center gap-1.5"><Calendar className="w-4 h-4 text-[var(--primary)]" /> {session.timeRange}</span>
                                                    <span className="bg-[var(--accent)] text-[var(--primary)] px-3 py-1 rounded-full text-[10px]">{session.class.roomCode}</span>
                                                    <span className={`px-3 py-1 rounded-full text-[10px] font-bold ${session.status === "ENDED" ? "bg-green-50 text-green-600" :
                                                        session.status === "IN_PROGRESS" ? "bg-blue-50 text-blue-600" :
                            ...
                            <div className="bg-white rounded-[2.5rem] border border-[var(--sidebar-border)] p-10 text-center">
                                ...
                                <p className="font-bold text-slate-400">Hôm nay không có lịch giảng dạy</p>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="space-y-6">
                    <h3 className="text-2xl font-black text-[var(--foreground)] tracking-tight px-2 flex items-center gap-2">
                        <Crown className="size-5 text-amber-500" /> Học viên xuất sắc
                    </h3>
                    <div className="bg-white rounded-[2.5rem] border border-[var(--sidebar-border)] p-6 shadow-sm space-y-4">
                        {dashboard?.topStudents && dashboard.topStudents.length > 0 ? (
                            dashboard.topStudents.map((student: any, i: number) => (
                                <div key={student.studentId} className="flex items-center justify-between p-3 hover:bg-[var(--accent)] rounded-2xl transition-all group border border-transparent hover:border-[var(--primary)]/20">
                                    <div className="flex items-center gap-4">
                                        <div className={`size-10 rounded-xl flex items-center justify-center text-sm font-black ${i === 0 ? "bg-amber-100 text-amber-600" :
                                        ...
                                        <div>
                                            <p className="text-sm font-black text-slate-700 group-hover:text-[var(--primary)] transition-colors">{student.fullName}</p>
                                            <p className="text-[10px] font-bold text-muted-foreground">{student.studentCode}</p>
                                        </div>
                                    </div>
                                    <Badge variant="outline" className={`rounded-full font-black ${i === 0 ? "border-amber-200 text-amber-600" :
                                        i === 1 ? "border-slate-200 text-slate-500" :
                    ...
                    {dashboard?.pendingScheduleChanges != null && dashboard.pendingScheduleChanges > 0 && (
                        <div className="bg-gradient-to-br from-[var(--primary)] to-[var(--secondary)] rounded-[2.5rem] p-6 text-white shadow-xl shadow-[var(--primary)]/30 relative overflow-hidden group">
                            <div className="relative z-10">
                                ...
                                <h4 className="text-xl font-black mb-1">Yêu cầu đổi lịch</h4>
                                <p className="text-xs font-bold opacity-80 mb-6 uppercase tracking-widest">Chờ duyệt</p>
                                <div className="flex justify-between items-end">
                                    <div>
                                        <p className="text-4xl font-black tracking-tighter">{dashboard.pendingScheduleChanges}</p>
            ...
            <ProfileDialog open={dialogOpen} onOpenChange={setDialogOpen} teacher={teacher} />
        </div>
    );
}
