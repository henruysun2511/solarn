...
import { useUpdateMyStudentProfile } from "@/queries/useStudentQuery";
import { Student, UpdateStudentProfileInput, updateStudentProfileSchema } from "@/schemas/student.schema";
...
export function ProfileDialog({ open, onOpenChange, student }: ProfileDialogProps) {
  const [publicId, setPublicId] = useState<string>("");
  const { handleUpload, isUploading } = useImageUpload();
  const deleteMediaMutation = useDeleteMedia();
  const updateProfileMutation = useUpdateMyStudentProfile();
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent data-role="student" className="!max-w-lg p-0 overflow-hidden rounded-[2rem] border-none shadow-2xl bg-slate-50/90 backdrop-blur-xl">
