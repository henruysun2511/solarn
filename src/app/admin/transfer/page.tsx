...
import { useGetTransferRequests, useProcessTransferRequest } from "@/queries/useRequestQuery";
import { TransferRequestParams } from "@/schemas/request.schema";
import { useState } from "react";
import { toast } from "sonner";
import { getColumns } from "./transfer-columns";
import { TransferFilter } from "./transfer-filter";

export default function AdminTransferPage() {
  const [params, setParams] = useState<TransferRequestParams>({
    page: 1,
    limit: 10,
  ...
  const { data, isLoading } = useGetTransferRequests(params);
  const processMutation = useProcessTransferRequest();
  ...
  const handleApprove = async (requestId: string) => {
    ...
    try {
      ...
      toast.success("Phê duyệt yêu cầu chuyển lớp thành công!");
    } catch (err: any) {
  ...
  return (
    <div data-role="admin" className="flex flex-col gap-6 min-h-screen">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <AdminPageTitle title="Xét duyệt chuyển lớp" subtitle="Quản lý" />
      </div>
