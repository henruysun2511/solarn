...
export default function AdminRoomPage() {
  const [params, setParams] = useState<RoomParams>({
    page: 1,
    limit: 10,
    sortOrder: SortOrder.DESC,
    sortBy: RoomSortBy.ROOM_CODE,
  });

  const { data, isLoading } = useGetRooms(params);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await deleteMutation.mutateAsync(deleteId, {
        ...
        onError: (error: any) => {
          handleError(error, "Không thể xóa phòng");
        }
      });
    } catch (error) {
  ...
  const handleSubmit = async (formData: RoomInput) => {
    try {
