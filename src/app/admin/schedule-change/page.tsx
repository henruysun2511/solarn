...
import { useGetScheduleChanges, useProcessScheduleChangeRequest } from "@/queries/useRequestQuery";
import { ScheduleChangeParams } from "@/schemas/request.schema";
import { useState } from "react";
import { toast } from "sonner";
import { getColumns } from "./schedule-change-columns";
import { ScheduleChangeFilter } from "./schedule-change-filter";

export default function AdminScheduleChangePage() {
  const [params, setParams] = useState<ScheduleChangeParams>({
    page: 1,
    limit: 10,
  ...
  const { data, isLoading } = useGetScheduleChanges(params);
  const processMutation = useProcessScheduleChangeRequest();
  ...
  const handleApprove = async (requestId: string) => {
    ...
    try {
      ...
      toast.success("Phê duyệt yêu cầu dời lịch thành công!");
    } catch (err: any) {
  ...
  return (
    <div data-role="admin" className="flex flex-col gap-6 min-h-screen">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <AdminPageTitle title="Xét duyệt dời lịch" subtitle="Quản lý" />
      </div>
