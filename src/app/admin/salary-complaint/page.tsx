...
import { useGetSalaryComplaints, useProcessSalaryComplaintRequest } from "@/queries/useRequestQuery";
import { SalaryComplaintParams } from "@/schemas/request.schema";
import { useState } from "react";
import { toast } from "sonner";
import { getColumns } from "./complaint-columns";
import { ComplaintFilter } from "./complaint-filter";

export default function AdminComplaintPage() {
  const [params, setParams] = useState<SalaryComplaintParams>({
    page: 1,
    limit: 10,
  ...
  const { data, isLoading } = useGetSalaryComplaints(params);
  const processMutation = useProcessSalaryComplaintRequest();
  ...
  const handleApprove = async (requestId: string) => {
    ...
    try {
      ...
      toast.success("Phê duyệt khiếu nại lương thành công!");
    } catch (err: any) {
  ...
  const handleReject = async (requestId: string) => {
    ...
    try {
      ...
      toast.success("Đã từ chối khiếu nại lương!");
    } catch (err: any) {
  ...
  return (
    <div data-role="admin" className="flex flex-col gap-6 min-h-screen">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <AdminPageTitle title="Khiếu nại lương" subtitle="Quản lý" />
      </div>
